    ...
    function initCreate(chooseBtn, options, callback) {
        ...
        WebUploader.Uploader.register({
            ...
            beforeSendFile: function (file) {
                ...
                obj.find(".note").show().html("计算文件特征...");
                obj.find('p.upload-state').hide();

                owner.md5File(file.source).fail(function () {
                    task.reject();
                }).progress(function (percentage) {
                    obj.find('.note').text('读取进度' + parseInt(percentage * 100) + "%");
                    ...
                    $.ajax(server, {
                        ...
                        if (response.exist) {
                            ...
                            obj.find('p.upload-state').show().attr('title', '正在上传').html('100%');
                            btn.html("开始上传");
                            btn.removeClass("stop");
                            btn.addClass("disabled");
                            obj.parents(ELEM).find(".webuploader-pick").removeClass("disabled");
                            task.reject();
                ...
                var task = new $.Deferred(), server = this.options.server;
                $.ajax({
                    type: "POST",
                    url: server,
                    data: {
    ...
    function uploadEvent(uploader, obj) {
        ...
        uploader.onFileQueued = function (file) {
            ...
            this.makeThumb(file, function (error, src) {
                if (error) {
                    $img.replaceWith('<div class="item-cover"><i class="iconfont icon-file-unknown"></i></div>');
                    return;
                }
                $img.attr('src', src);
            }, thumbSize, thumbSize);
        ...
        uploader.onUploadProgress = function (file, percentage) {
            ...
            $li.find(".note").remove();
            $li.find('p.upload-state').show().attr('title', '正在上传').html(parseInt(percentage * 100) + '%');
